function [t,xp_avg_ADE,xp_var_ADE,xp_skew_ADE,xp_kur_ADE,BTCs,D_coef,x,C] = model_ADE(time_sim,mean_xp_sim,var_xp_sim,diameter,dxups)
...
global_D_coef      = mean(D_coef);
global_mean_vp_sim = mean_vp_sim;


% Space and time discretization *******************************************
dx = dxups;
LR = max(50*diameter,double(solve(['0.5*(1+erf((x-',num2str(max(mean_xp_sim)),')/(sqrt(2)*sqrt(',num2str(max(var_xp_sim)),'))))-0.99999'])));
LL = min(-50*diameter,double(solve(['0.5*(1+erf((x-',num2str(max(mean_xp_sim)),')/(sqrt(2)*sqrt(',num2str(max(var_xp_sim)),'))))-0.00001']))); % LR and LL are arbitrary lengths. Warning: ensure that the particles are not arriving to x=LR neither x=LR. Check this by comparing the real and modeled meanpos.
...
if nd == 1
    ...
    hist_BTCs = compute_btc(C,x,diameter,hist_BTCs);                       % Compute the breakthrough curves (see Sund et. al, 2015)
